...
public class WayFinderBackEnd<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			if (copyPath == null) {
				throw new NoSuchElementException("Path is null.");
			}
			if (extendBy == null) {
				throw new NoSuchElementException("Edge is null.");
			}
		...
		public String toString() {
			return "start: " + this.start.data + " end: " + this.end.data + " distance: " + this.distance;
		}
	}
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		//makes sure the graph contains the specified data
		if (!containsVertex(start) || !containsVertex(end)) {
			throw new NoSuchElementException("Start or end is not in graph.");
		}

		ArrayList<Vertex> visited = new ArrayList<Vertex>(); //tracks visited nodes
		PriorityQueue<Path> pq = new PriorityQueue<Path>(); //holds pending paths
		PriorityQueue<Path> answer = new PriorityQueue<Path>(); //holds paths w/ end
		Vertex next = vertices.get(start);
		Path path = new Path(next);
		pq.add(path); //first node visited is start node
		while (!pq.isEmpty()) {
			int count = next.edgesLeaving.size();
			//iterates through each of the nodes edges
			for (int i = 0; i < count; i++) {

				Path cpPath = new Path(path, next.edgesLeaving.get(i));
				
				//makes sure the target of the edge was not visited already
				if (!visited.contains(next.edgesLeaving.get(i).target)) {
						pq.add(cpPath);
						//adds paths that end with the desired end
						if (cpPath.end.data.equals(end)) {
							answer.add(cpPath);
						}
				}
			}
			//marks the path's end node as visited after tracking each of its edges
